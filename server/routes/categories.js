...
module.exports = (app) => {
    app.post('/categories/list', async ({ body }, res) => {
        ...
        categories.find(body.parent ? { parent: body.parent } : {}, function (dbErr, dbRes) {
            connectDB(false);
            res.send(dbRes);
        });
    });
    ...
    app.post('/categories/edit', async ({ body }, res) => {
        ...
        categories.findOneAndUpdate({ _id: body._id }, { ...body, image: newImage }, function (dbErr) {
            connectDB(false);
            res.send('ok');
        });
    });
    app.post('/categories/remove', async ({ body }, res) => {
        ...
        categories.remove({ _id: body._id }, function (dbErr) {
            ...
            if (fs.existsSync(`images/categories/${removeFile}`) && body.image !== '') {
