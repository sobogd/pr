...
const edit = async (body) => {
  ...
  const updateResult = await slides.findOneAndUpdate(
    { _id: body._id },
    { ...body, image1, image2 }
  );
  return updateResult;
...
module.exports = (app) => {
   ...
   */
  app.post("/api/slides/list", async ({ body }, res) => {
   ...
   */
  app.post("/api/slides/edit", async ({ body }, res) => {
    await connectDB(true);
    try {
   ...
   */
  app.post("/api/slides/remove", async ({ body }, res) => {
    await connectDB(true);
    try {
   ...
   */
  app.post("/api/slides/add", async ({ body }, res) => {
    await connectDB(true);
    try {
